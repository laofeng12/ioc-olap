    ...
    public List<DataLakeQueryJobStatusVo> batchQueryJobStatus(List<DataLakeJobQueryParam> params) throws Exception {
        ...
        if (object != null && object.containsKey("rows") && (array = object.getJSONArray("rows")) != null){
            array.forEach(s->{
                ...
                BeanUtils.copyProperties(item,vo);
                optional.ifPresent(dataLakeJobQueryParam -> vo.setCubeName(dataLakeJobQueryParam.getCubeName()));
            });
        }
        return list;
    }
    ...
    public List<String> updateCubeStatus(List<DataLakeQueryJobStatusVo> result) throws Exception {
        ...
        if (result != null && result.size() > 0){
            ...
            map.forEach((key,value)->{
                int status = -1;
                //统计成功同步的总数
                long suc = value.stream().filter(s->s.getStatus() == 4).count();
                //统计同步失败的总数
                long failed = value.stream().filter(s->s.getStatus() == 5).count();
                if (value.size() == suc){
                    //全部表同步成功，cube才能设为构建中
