...
public interface OlapShareRepository extends DynamicJpaRepository<OlapShare, Long>, OlapShareRepositoryCustom{
    ...
    @Query(value = "select new com.openjava.platform.dto.ShareUserDto(p.ID,p.FK_ID,p.FK_TYPE,p.SHARE_USER_ID,p.CREATE_TIME,p.CREATE_ID,p.CREATE_NAME,q.FULLNAME )  from OLAP_SHARE p inner join SYS_USER q on p.shareUserId=q.USERID where p.FK_TYPE = :sourceType and p.FK_ID = :sourceId and p.CREATE_ID = :userId",nativeQuery = true)
